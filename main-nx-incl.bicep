...
param deployJumpboxWindowsAddOns bool = false
...
param deploySecureBatch bool = false

@description('Indicate if the NxOv Demo should be deployed.')
param deployNxOneView bool = true
...
var rgNxOv        = 'rg-${environment}-${prefix}-nxov'
...
//---------------------------  Deploy the Nx One View Demo-----------------------------------------------------------------

module deployNxOneViewDemo './modules/Demos/Demo-NxOneView/demoNxOneView.bicep' = if (deployNxOneView) { 
  scope: resourceGroup(rgNxOv)
  name: 'dpl-${uniqueString(deployment().name,deployment().location)}-NxOv'
  params: {
    rgSpoke: rgSpoke02
    environment: environment
    vNetObject: vNetSpoke02Param
    rgHub: rgHub
    prefix: prefix
    administratorLogin: adminUserName
    administratorLoginPassword: adminPassword
    vmNodeSize: jumpboxLinuxVmSize
  }
  dependsOn: [
    hubSpokeNetwork
  ]
