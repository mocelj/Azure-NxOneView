...
param multipleWritelocations bool = true
...
//modr
@description('Optional. Assigned Throughput')
...
module databaseAccount_privateEndpoints './nested/nested_privateEndpoints.bicep' = [for (privateEndpoint, index) in privateEndpoints: {
  name: '${uniqueString(deployment().name, location)}-DocumentDB-PrivateEndpoint-${index}'
  params: {
    privateEndpointResourceId: databaseAccount.id
